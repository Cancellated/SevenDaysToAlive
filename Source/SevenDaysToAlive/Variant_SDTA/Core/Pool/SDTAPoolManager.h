 ...
 * 
 * 核心功能：
 * 1. 管理多种类型的对象池，支持Actor和UObject类型
 * 2. 提供对象的高效获取和回收机制，减少频繁创建和销毁对象的性能开销
 * 3. 支持服务器端权威管理，确保网络一致性
 * 4. 提供灵活的对象池配置（初始大小、最大大小等）
 * 5. 支持对象状态重置和生命周期管理
 * 
 * 设计理念：
 * - 采用模板化设计，支持多种对象类型
 * - 实现服务器端权威制，确保网络环境下的一致性
 * - 提供简洁易用的接口，便于集成到游戏系统中
 * - 支持对象的自动状态重置和激活/休眠管理
 * 
 * 使用场景：
 * - 敌人对象的高效回收和复用
 * - 项目ilectile等频繁创建/销毁的游戏对象
 * - UI元素的动态管理
 * - 任何需要高性能对象管理的场景
 */
